...
def tSNE_recommend(title, sorted_movies_df, range=20, n=5):
    ...
    if movie_index:
        ...
        return 'Can\'t Find this Movie, Recommend Generally'
...
def main(movie, user_id):
    pred = pd.read_csv('rating_hat_svd.csv')
    pred.set_index('userId', inplace=True)

    movies_sorted = pd.read_csv('tSNE_sorted.csv')

    similar_movie_df = tSNE_recommend(movie, movies_sorted)

    if type(similar_movie_df) == str:

        general_rec=user_general_rec(user_id, pred, movies_sorted)
        general_rec.insert(0,similar_movie_df)
        return general_rec
